...
### Attachment storage (Cloudflare R2) configuration

Chat media never travels through the signaling server: `POST
/attachments/presign` hands the client a signed `PUT`, and `GET
/attachments/download` exchanges a stored reference for a short-lived signed
`GET` after checking the caller's session, the block list and the conversation
scope embedded in the object key.

#### The bucket is what decides whether that check means anything

**R2 public access is bucket-level, not per-prefix.** A bucket bound to a
custom domain or exposed through its `r2.dev` URL serves every object in it to
anyone who has the URL:

```bash
# A bucket exposed via custom domain — no session, no signature, 200 OK.
curl -sSI "https://<your-r2-domain>/chatblobs/<scope>/<uuid>.zip"
```

The UUID in the key is **not** a security boundary: attachment URLs travel
through push payloads, exports, screenshots and client logs, and one that
leaks is a permanent public link for as long as the object exists. There is no
way to keep a "private prefix" inside a publicly-exposed bucket.

**Recommended configuration:** no public binding at all — no custom domain, no
`r2.dev` — with bytes reachable only through `GET /attachments/download`. The
server warns at startup (naming `R2_PUBLIC_BASE_URL`) when it detects a
configuration where attachments are publicly readable.

#### Private bucket (`R2_BUCKET_PRIVATE`)

If the current bucket must stay public (an existing custom domain, other
assets in the same bucket), point new attachments at a second bucket that has
no public binding:

```dotenv
R2_BUCKET=shared-assets            # existing, possibly public bucket
R2_BUCKET_PRIVATE=chat-attachments # no custom domain, no r2.dev
```

Uploads then go to `R2_BUCKET_PRIVATE`; downloads resolve against it first and
fall back to `R2_BUCKET`, so attachments uploaded before the switch keep
working. The same R2 API token must be able to reach both buckets. Signing is
plain SigV4 and needs no public binding on either.

#### Changing `R2_PUBLIC_BASE_URL`

Stored attachment URLs carry whichever base URL was configured when the message
was sent, so a deployment that moves from `https://pub-<id>.r2.dev` to a custom
domain ends up with two generations of URL pointing at the *same* object key
(`chatblobs/<scope>/<uuid>.<ext>`).

Downloads resolve by that key path rather than by an exact base-URL match, so
**no database change is required** — older messages keep downloading. The
accepted hosts are still bounded: the configured base URL, anything listed in
the optional comma-separated `R2_PUBLIC_BASE_URL_LEGACY`, and R2's own public
hostnames. An arbitrary host is refused, whatever its path looks like.

```dotenv
# Optional: retired custom domains/CDN hostnames this deployment published under.
R2_PUBLIC_BASE_URL_LEGACY=https://media.old.example,https://cdn.older.example
```

To normalise the rows anyway, rewrite the host in place (dry run first — it
writes nothing without `--apply`):

```bash
cd /opt/studious-robot/server
DATABASE_URL_DIRECT=<direct-url> npm run db:rewrite-attachment-hosts
DATABASE_URL_DIRECT=<direct-url> npm run db:rewrite-attachment-hosts -- --apply
```

Only the host is rewritten, inside a single transaction; a URL on a host this
deployment never published under is reported and left alone.
