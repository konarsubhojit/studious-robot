 ...
 *
 * `haptics.ts` already read this setting, but kept it in a module-level
 * variable that only it could see, so every *visual* animation in the app ran
 * regardless: the incoming-call pulse, the call-chrome fade, the message-bubble
 * spring and the overlay transitions all ignored a preference the app had
 * already been told about. This hook is the shared reader.
 *
