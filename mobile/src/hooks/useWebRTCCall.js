...
export default function useWebRTCCall() {
  ...
  const [isSpeakerEnabled, setIsSpeakerEnabled] = useState(
    DEFAULT_SETTINGS.speakerEnabledByDefault,
  );
  ...
  const leaveRoom = useCallback(
    (nextStatus = 'Disconnected', severity = 'info') => {
    ...
    try {
      ...
      if (typeof onTransportUpgrade === 'function') {
        onTransportUpgrade.call(socket.io.engine, 'upgrade', transport => {
          logInfo('Socket.IO transport upgrade', {
      ...
      socket.on('peer-joined', async () => {
        ...
        try {
          ...
          logInfo('Offer created and sent', {
            sdpType: offer?.type || 'unknown',
          });
        } catch (error) {
        ...
        try {
          ...
          for (const c of buffered) {
            try {
              ...
              logWarn('Failed to add buffered ICE candidate', {
                message: err?.message,
              });
            }
          }
          ...
          logInfo('Answer created and sent', {
            sdpType: answer?.type || 'unknown',
          });
        } catch (error) {
      ...
      socket.on('connect_error', error => {
        ...
        setIsInRoom(false);
        updateStatus(`Unable to connect: ${error?.message || 'Unknown error'}`, 'error');
      });
    } catch (error) {
  ...
  const chooseAudioOutput = useCallback(
    async route => {
      try {
        ...
        logError('Failed to choose audio route', {
          route,
          message: error?.message,
        });
  ...
  const handleAutoLightingToggle = useCallback(() => {
    ...
    updateStatus(nextValue ? 'Auto camera lighting enabled' : 'Auto camera lighting disabled');
  ...
  const handleSpeakerDefaultToggle = useCallback(() => {
    ...
    updateStatus(nextValue ? 'Speaker default enabled' : 'Speaker default disabled');
  ...
  const handleDeveloperModeToggle = useCallback(() => {
    ...
    updateStatus(nextValue ? 'Developer mode enabled' : 'Developer mode disabled');
  ...
  const handleExportLogs = useCallback(async () => {
    try {
      ...
      if (result.success) {
        ...
        updateStatus(`Failed to export logs: ${result.error?.message || 'Unknown error'}`, 'error');
      }
    } catch (error) {
      logError('Unexpected export logs failure', error);
      updateStatus(`Failed to export logs: ${error?.message || 'Unknown error'}`, 'error');
    }
  }, [isInRoom, localStream, remoteStream, roomId, updateStatus, signalingUrl, status.message]);
