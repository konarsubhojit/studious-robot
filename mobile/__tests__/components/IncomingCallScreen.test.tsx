...
describe('IncomingCallScreen', () => {
  ...
  test('displays the caller ID', () => {
    ...
    const nodes = tree.root.findAll((/** @type {any} */ n: any) => n.props.testID === 'incoming-caller-id');
  ...
  test('derives avatar initials from multi-part caller IDs', () => {
    ...
    expect(tree.root.findAll((/** @type {any} */ node: any) => node.props.children === 'CB').length).toBeGreaterThanOrEqual(
      1,
    );
  });
  ...
  test('renders Accept and Decline icon buttons', () => {
    ...
    const testIDs = buttons.map((/** @type {any} */ b: any) => b.props.testID);
  ...
  test('calls onAccept when Accept button is pressed', () => {
    ...
    act(() => {
      const acceptBtn = tree.root
        ...
        .find((/** @type {any} */ b: any) => b.props.testID === 'incoming-accept');
  ...
  test('calls onDecline when Decline button is pressed', () => {
    ...
    act(() => {
      const declineBtn = tree.root
        .findAllByType('IconButton')
        .find((/** @type {any} */ b: any) => b.props.testID === 'incoming-decline');
  ...
  test('shows countdown when ringTimeoutAt is provided', () => {
    ...
    const nodes = tree.root.findAll((/** @type {any} */ n: any) => n.props.testID === 'incoming-countdown');
    expect(nodes.length).toBeGreaterThanOrEqual(1);
  });
  ...
  test('renders a two-minute ring window as m:ss', () => {
    ...
    const [node] = tree.root.findAll((/** @type {any} */ n: any) => n.props.testID === 'incoming-countdown');
  ...
  test('hides countdown when ringTimeoutAt is absent', () => {
    ...
    expect(tree.root.findAll((/** @type {any} */ n: any) => n.props.testID === 'incoming-countdown')).toHaveLength(0);
  });
