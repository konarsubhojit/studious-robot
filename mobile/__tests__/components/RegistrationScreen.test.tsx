...
describe('RegistrationScreen', () => {
  test('renders without throwing', () => {
    ...
    expect(tree.root.findAll((n: any) => n.props.testID === 'registration-username-input')).toHaveLength(
      2,
  ...
  test('renders email and password inputs', () => {
    ...
    expect(
      tree.root.findAll((n: any) => n.props.testID === 'registration-email-input'),
    ).toHaveLength(2);
    expect(
      tree.root.findAll((n: any) => n.props.testID === 'registration-password-input'),
    ).toHaveLength(2);
  });
  ...
  test('Get Started button is disabled when input is empty', () => {
    ...
    const btn = tree.root
      ...
      .find((button: any) => button.props.testID === 'registration-email-register');
  ...
  test('submits email registration fields', () => {
    ...
    act(() => {
      tree.root
        .findAll((n: any) => n.props.testID === 'registration-username-input')[0]
        .props.onChangeText(' alice ');
      tree.root
        .findAll((n: any) => n.props.testID === 'registration-email-input')[0]
        .props.onChangeText(' alice@example.com ');
      tree.root
        .findAll((n: any) => n.props.testID === 'registration-password-input')[0]
  ...
  test('disables provider buttons when providers are unconfigured', () => {
    ...
    const googleButton = tree.root
      .findAllByType('AppButton')
      .find((button: any) => button.props.testID === 'registration-google');
    const microsoftButton = tree.root
      .findAllByType('AppButton')
      .find((button: any) => button.props.testID === 'registration-microsoft');
  ...
  test('surfaces a failed sign-in as an error state that retries the last attempt', () => {
    ...
    expect(
      tree.root.findAll((n: any) => n.props.testID === 'registration-error').length,
    ).toBeGreaterThanOrEqual(1);
    expect(tree.root.findAll((n: any) => n.props.testID === 'registration-error-action')).toHaveLength(0);
    ...
    const retry = tree.root.find(
      (n: any) => n.props?.testID === 'registration-error-action' && typeof n.props.onPress === 'function',
    );
    act(() => {
      retry.props.onPress();
    });
