...
6. Continue the checkpointed `useCallFlow` effect-hook extraction in
   `docs/CALLFLOW_EXTRACTION.md` (next wiring: CP6 `useAnswerPath`; the CP6
   file already exists as an unwired scaffold) and split
...
CP4 `useLocalMedia` and CP5 `useSignalingSocket` have landed,
`useCallFlow.ts` is down to 2,909 lines, and the CP6 hook file
(`useAnswerPath`) exists as an unwired scaffold. Start by wiring CP6
`useAnswerPath`; teardown's verdict, whichever way it goes, is recorded back
here after CP6.
