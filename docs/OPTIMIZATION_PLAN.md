...
6. Continue the checkpointed `useCallFlow` effect-hook extraction in
   `docs/CALLFLOW_EXTRACTION.md` (next wiring: CP4 `useLocalMedia`; CP4–CP6
   files already exist as unwired scaffolds) and split
...
`useCallAudioRouting`, CP2 `useConnectionQuality` and CP3 `usePeerConnection`
have landed, `useCallFlow.ts` is down to 3,560 lines, and CP4–CP6 hook files
(`useLocalMedia`, `useSignalingSocket`, `useAnswerPath`) exist as unwired
scaffolds. Start by wiring CP4 `useLocalMedia`; teardown's verdict, whichever
