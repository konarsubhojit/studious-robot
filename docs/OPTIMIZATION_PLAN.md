...
6. Continue the checkpointed `useCallFlow` effect-hook extraction in
   `docs/CALLFLOW_EXTRACTION.md` (next wiring: CP5 `useSignalingSocket`; CP5–CP6
...
and CP4 `useLocalMedia` have landed, `useCallFlow.ts` is down to 3,452 lines,
and CP5–CP6 hook files (`useSignalingSocket`, `useAnswerPath`) exist as
unwired scaffolds. Start by wiring CP5 `useSignalingSocket`; teardown's verdict,
whichever way it goes, is recorded back here after CP6.
