...
| `WEB_PUBSUB_CONNECTION_STRING` | _(unset)_ | **Optional, experimental.** Connection string of an Azure Web PubSub resource in **Socket.IO mode**. When set, socket fan-out is attached through `@azure/web-pubsub-socket.io` instead of the Socket.IO Redis adapter and `/health` reports `socketTransport: "web-pubsub"`. A missing package or a failed initialisation logs loudly and falls back to the Redis adapter. Redis stays mandatory for the call registry, sessions and the cache either way (see `src/lib/socketAdapter.ts`, `docs/AZURE_SETUP.md` Part 2). |
| `WEB_PUBSUB_HUB` | `signaling` | Hub name used when the Web PubSub transport is active. Must match across instances. |
