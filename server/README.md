...
| `GET /account/export` | `limit` (message page, 1–100, default `50`), `before` (message cursor), `callLimit` (1–100, default `50`), `callOffset` (default `0`) | `200 { schemaVersion, exportedAt, profile, messages, calls, callEvents, devices, blocks, audit, pagination }` | Machine-readable export scoped only to the bearer session. Includes message tombstones and attachment URLs, participant call history/events, outbound blocks, and audit entries involving the account. Excludes `authUid`, push tokens, session ids, and attachment bytes/metadata. Limited to one request per account per day by default. |
