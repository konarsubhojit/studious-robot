...
/**
 * Persist a device push-token registration (or its cleared state on
 * unregister) so it survives restarts.  The caller passes the already-upserted
 * in-memory device record; a `null` `pushProvider`/`pushToken` represents an
 * unregistration.
 *
 * @param {object|null} db
 * @param {object} device
 * @param {'registration'|'unregistration'} [action]
 * @returns {Promise<void>}
 */
  ...
  try {
    await db.insert(devicesTable).values(values).onConflictDoUpdate({
      ...
      set: {
        ...
        lastRegisteredAt: values.lastRegisteredAt,
        lastUnregisteredAt: values.lastUnregisteredAt,
