...
function createTurnCredentialsRouter({ state, fetchImpl = fetch, env = process.env }: {
  ...
  router.get(API_ROUTES.TURN_CREDENTIALS, async (req, res) => {
    ...
    const now = Date.now();
    const cloudflare = await cloudflareCredentials(now);
    if (cloudflare) {
      res.set('X-Turn-Credential-Expires-At', cloudflare.expiresAt.toISOString());
      res.json(cloudflare.iceServers);
      return;
    }
    ...
    const hmac = hmacCredentials(session.userId, now);
    if (hmac) {
      res.set('X-Turn-Credential-Expires-At', hmac.expiresAt.toISOString());
      res.json(hmac.iceServers);
      return;
    }
