...
test('message.react converges across every device of both participants', async (t) => {
  ...
  const onTablet = new Promise<any>((resolve) => aliceTablet.once('message.reaction', resolve));
  const onSender = new Promise<any>((resolve) => alice.once('message.reaction', resolve));
...
test('a message with an unknown type stays renderable by an older client', () => {
  ...
  assert.equal(messageTypeOf(({ body: 'legacy' } as any)), 'text');
  assert.equal(describeMessagePreview(({ body: 'legacy' } as any)), 'legacy');
